...
### 6. Настройка DNS
В панели управления вашего домена (где вы покупали домен: reg.ru, namecheap, godaddy и т.д.) создайте A-запись:

```
Тип: A
Имя: taxi    (если хотите taxi.example.com)
     или @   (если хотите просто example.com)
Значение: ГЛОБАЛЬНЫЙ (публичный) IP-адрес вашего сервера
TTL: 300
```

**Как узнать глобальный IP сервера:**
```bash
...
| `app` | app.example.com |
| `@` или пусто | example.com |

**Важно:** Локальный IP (192.168.x.x, 10.x.x.x, 127.0.0.1) НЕ подходит! Нужен именно глобальный IP, который виден из интернета.
